...
public class Element {
    ...
    private static final ThreadLocal<FindWebElement> findWebElement = ThreadLocal.withInitial(() -> (FindWebElement) ObjectSupplier.instanceOf(FindWebElement.class));
    ...
    public static WebElement element(By byLocator, boolean value) {
        return findWebElement.get().webElement(byLocator, value);
    }

    protected static void remove() {
        findWebElement.remove();
    }
    ...
    public WebElement getElement(List<By> locatorSet, boolean visibility, boolean isMultiple) {
        ...
        try {
            if (locatorSet.size() > 1) {
                ...
                element = findWebElement.get().webElement(locators.get(0), visibility);
                ...
                element = findWebElement.get().webElements(locatorSet.get(0), Integer.parseInt(getData(ElementData.INDEX)), visibility);
            } else if (locatorSet.size() == 1) {
                element = findWebElement.get().webElement(locatorSet.get(0), visibility);
            } else {
    ...
    public List<WebElement> getElements(List<By> locatorSet, boolean visibility) {
        ...
        try {
            if (locatorSet.size() > 1) {
                ...
                elements = findWebElement.get().webElements(locatorSet.get(0), visibility);
            }
        } catch (WebDriverException e) {
            logger.error(e.getMessage());
